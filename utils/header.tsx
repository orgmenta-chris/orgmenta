...
export const getHeaderDimensions = () => {
  ...
  if (platform_os === 'web') { return { height: 55, padding: 0 } }
  if (platform_os === 'ios') { return { height: 70, padding: 0 } } // iOS uses safeAreaView instead (so we just need padding, no actual statusbar height needed)
  if (platform_os === 'android') { return { height: 85, padding: 5 }  } // this is a hack to ensure that it doesn't cover the statusbar on android. This may not cover some custom roms. (statusbar height + padding needed)
  else { return { height: 70, padding: 0 } } // Unknown as to whether this fallback would ever be used (depends on future changes to react-native Platform)
}
...
export const ViewHeaderSection = ({ children, flex, padding }:TypeHeaderSection) => {
  return (

...
export const ViewWidgetMain = ({children, title,summary, collapsible, help, direction,backgroundColor, height=20}:any) => { 
    ...
    return (
        <Pressable
          ...
          style={{flex:1,backgroundColor:backgroundColor, minHeight:100,minWidth:100, flexDirection: direction}}
        >
          ...
          { showInfo && 
            <View style={{flexDirection:'row',backgroundColor:'gray', position:'absolute', top:0, left:0, width:'100%'}}>
          ...
          {showOptions && 
            <View style={{position: 'absolute', flexDirection:'row',top:0, width:"100%"}}>
              ...
              { ( title || summary) && 
                <Pressable
                  onHoverIn={()=>{setOptions(true);setInfo(true)}}
                  onHoverOut={()=>{setInfo(false)}}
                  onPress={()=>{setOptions((oldValue)=>!oldValue);setInfo((oldValue)=>!oldValue)}}

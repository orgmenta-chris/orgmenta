...
import { useNavigate, useLocation, BrowserRouter, BrowserRouterProps, Routes, RoutesProps, Route, RouteProps, Navigate, NavigateProps, Link as DLink, LinkProps as DLinkProps } from "react-router-dom" 
import { NativeRouter, NativeRouterProps, Link as NLink, LinkProps as NLinkProps } from "react-router-native"
...
export const ViewRouterProvider = UtilityPlatformMain.OS === "web" ? BrowserRouter : NativeRouter
...
export const ViewRouterLinkthemed = ({ children, theme_token, style, ...rest }: TypeRouterLinkthemed) => {
    ...
    const stylesheetMerged = mergeStylesheetMain(theme?.style, isStyleObject ? (style as TypeThemeMain) : {});
...
export const ViewRouterLostpage = () => {
  return (
    <ViewPageMain>
      <Text style={{ margin: 20, fontSize:30, fontWeight:'800', textAlign:'center' }}>404 Not Found</Text>
      <ViewRouterLinkthemed to={'/'}>
        <ViewTypographyTextthemed style={{ fontSize: 24, flex:1, alignSelf:'center' }}>Home</ViewTypographyTextthemed>
